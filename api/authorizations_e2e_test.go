...
func TestAuthorizationsAPI(t *testing.T) {
	...
	listRes, err = authAPI.FindAuthorizationsByUserName(context.Background(), "admin")
	require.Nil(t, err)
	require.NotNil(t, listRes)
	assert.Len(t, *listRes, 2)
	...
	listRes, err = authAPI.FindAuthorizationsByOrgName(context.Background(), "memory")
	require.Nil(t, err)
	require.NotNil(t, listRes)
	assert.Len(t, *listRes, 2)

	user, err := client.UsersAPI().FindUserByName(context.Background(), "admin")
	require.Nil(t, err)
